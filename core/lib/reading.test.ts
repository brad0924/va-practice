...
describe('AI 預填採用 acceptPrefill', () => {
  ...
  /** 組整份回覆。`termKana` 為何是佔位值見 `reading.ts`；被忽略由下面三個案例守著。 */
  const aiReply = (...runs: unknown[]) => ({ termKana: 'かな', runs });

  it('單一漢字串：焦がす', () => {
    expect(acceptPrefill('焦がす', aiReply(ai(true, ['焦', 'こ'])))).toEqual([
      { start: 0, cells: [{ kanji: '焦', reading: 'こ' }] },
    ]);
  });

  it('逐字分配：吹雪 兩格', () => {
    expect(acceptPrefill('吹雪', aiReply(ai(true, ['吹', 'ふ'], ['雪', 'ぶき'])))).toEqual([
      {
        start: 0,
        cells: [
          { kanji: '吹', reading: 'ふ' },
  ...
  it('拆不開的整串一格：剃刀', () => {
    expect(acceptPrefill('剃刀', aiReply(ai(false, ['剃刀', 'かみそり'])))).toEqual([
      { start: 0, cells: [{ kanji: '剃刀', reading: 'かみそり' }] },
    ]);
  });

  it('說拆不開卻還是拆了：程式自己合併回去，讀音接起來', () => {
    expect(acceptPrefill('剃刀', aiReply(ai(false, ['剃', 'かみ'], ['刀', 'そり'])))).toEqual([
      { start: 0, cells: [{ kanji: '剃刀', reading: 'かみそり' }] },
    ]);
  });

  it('說拆得開就照它給的格數，不代為合併', () => {
    expect(acceptPrefill('吹雪', aiReply(ai(true, ['吹雪', 'ふぶき'])))).toEqual([
  ...
  it('多串：考え込む 兩串，start 各自正確', () => {
    expect(
      acceptPrefill('考え込む', aiReply(ai(true, ['考', 'かんが']), ai(true, ['込', 'こ']))),
    ).toEqual([
      { start: 0, cells: [{ kanji: '考', reading: 'かんが' }] },
      { start: 2, cells: [{ kanji: '込', reading: 'こ' }] },
    ]);
  ...
  it('不採信 AI 的位置資訊，start 一律自己算', () => {
    expect(
      acceptPrefill('ざるを得ない', {
        termKana: 'ざるをえない',
        runs: [{ splittable: true, cells: [{ kanji: '得', reading: 'え', start: 99 }] }],
      }),
    ).toEqual([{ start: 3, cells: [{ kanji: '得', reading: 'え' }] }]);
  });

  it('片假名讀音放行', () => {
    expect(acceptPrefill('缶', aiReply(ai(true, ['缶', 'カン'])))).toEqual([
      { start: 0, cells: [{ kanji: '缶', reading: 'カン' }] },
    ]);
  });

  it('拒絕：串數與詞條對不上', () => {
    expect(acceptPrefill('考え込む', aiReply(ai(true, ['考', 'かんが'])))).toBeNull();
    expect(
      acceptPrefill('焦がす', aiReply(ai(true, ['焦', 'こ']), ai(true, ['込', 'こ']))),
    ).toBeNull();
  });

  it('拒絕：AI 改字、漏字、多字', () => {
    expect(acceptPrefill('吹雪', aiReply(ai(false, ['吹雨', 'ふぶき'])))).toBeNull();
    expect(acceptPrefill('吹雪', aiReply(ai(false, ['吹', 'ふぶき'])))).toBeNull();
    expect(acceptPrefill('吹雪', aiReply(ai(false, ['大吹雪', 'おおふぶき'])))).toBeNull();
  });

  it('拒絕：讀音含羅馬字、漢字或數字', () => {
    expect(acceptPrefill('焦がす', aiReply(ai(true, ['焦', 'ko'])))).toBeNull();
    expect(acceptPrefill('焦がす', aiReply(ai(true, ['焦', '焦'])))).toBeNull();
    expect(acceptPrefill('焦がす', aiReply(ai(true, ['焦', 'こ1'])))).toBeNull();
  });

  it('拒絕：任一格讀音為空字串', () => {
    expect(acceptPrefill('吹雪', aiReply(ai(true, ['吹', 'ふぶき'], ['雪', ''])))).toBeNull();
  });

  it('拒絕：任一格漢字為空字串', () => {
    expect(acceptPrefill('吹雪', aiReply(ai(true, ['吹雪', 'ふぶき'], ['', 'き'])))).toBeNull();
  });

  it('拒絕：形狀不對', () => {
    expect(acceptPrefill('焦がす', null)).toBeNull();
    expect(acceptPrefill('焦がす', '焦[こ]がす')).toBeNull();
    expect(acceptPrefill('焦がす', aiReply([{ kanji: '焦', reading: 'こ' }]))).toBeNull();
    expect(acceptPrefill('焦がす', aiReply(ai(true)))).toBeNull();
    expect(acceptPrefill('焦がす', aiReply({ splittable: true, cells: ['こ'] }))).toBeNull();
    expect(
      acceptPrefill('焦がす', aiReply({ splittable: true, cells: [{ kanji: '焦' }] })),
    ).toBeNull();
    expect(
      acceptPrefill('焦がす', aiReply({ splittable: true, cells: [{ kanji: 1, reading: 'こ' }] })),
    ).toBeNull();
  });

  it('拒絕：最外層還是舊的陣列形狀', () => {
    expect(acceptPrefill('焦がす', [ai(true, ['焦', 'こ'])])).toBeNull();
  });

  it('拒絕：最外層沒有 runs、或 runs 不是陣列', () => {
    expect(acceptPrefill('焦がす', { termKana: 'こがす' })).toBeNull();
    expect(acceptPrefill('焦がす', { termKana: 'こがす', runs: '焦' })).toBeNull();
  });

  it('拒絕：缺 splittable 或不是布林', () => {
    expect(acceptPrefill('焦がす', aiReply({ cells: [{ kanji: '焦', reading: 'こ' }] }))).toBeNull();
    expect(
      acceptPrefill('焦がす', aiReply({ splittable: 'true', cells: [{ kanji: '焦', reading: 'こ' }] })),
    ).toBeNull();
  });

  it('termKana 缺漏照樣採用：它是鷹架，不參與驗證', () => {
    expect(acceptPrefill('焦がす', { runs: [ai(true, ['焦', 'こ'])] })).toEqual([
      { start: 0, cells: [{ kanji: '焦', reading: 'こ' }] },
    ]);
  });

  it('termKana 是空字串或不是字串，照樣採用', () => {
    const expected = [{ start: 0, cells: [{ kanji: '焦', reading: 'こ' }] }];
    expect(acceptPrefill('焦がす', { termKana: '', runs: [ai(true, ['焦', 'こ'])] })).toEqual(
      expected,
    );
    expect(acceptPrefill('焦がす', { termKana: 42, runs: [ai(true, ['焦', 'こ'])] })).toEqual(
      expected,
    );
  });

  it('termKana 跟各格假名接不起來也照樣採用，不對帳', () => {
    expect(
      acceptPrefill('吹雪', {
        termKana: 'ぜんぜんちがう',
        runs: [ai(true, ['吹', 'ふ'], ['雪', 'ぶき'])],
      }),
    ).toEqual([
      {
        start: 0,
  ...
  it('往返：採用後 toMarkup 再 parseReading 還原同一組漢字與讀音', () => {
    const runs = acceptPrefill(
      '書き下ろす',
      aiReply(ai(true, ['書', 'か']), ai(true, ['下', 'お'])),
    );
