...
describe('問 Gemini 讀音', () => {
  ...
  it('等太久沒回覆：訊息講的是秒數，不是換算前的毫秒', async () => {
    ...
    try {
      ...
      const settled = expect(asking).rejects.toThrow('等超過 10 秒沒有回覆');
  ...
  it('連不上：不是被我們取消的失敗，就講連不上', async () => {
    ...
    await expect(askReading(KEY, TERM, offline)).rejects.toThrow('連不上 Gemini');
  });

  it('非 2xx 且挖得到原因：狀態碼與那句原因一起帶出來', async () => {
    const doFetch = responds(() => json({ error: { message: 'Quota exceeded' } }, 429));

    await expect(askReading(KEY, TERM, doFetch)).rejects.toThrow('Gemini 回了 429：Quota exceeded');
  });

  it('非 2xx 但挖不到原因：狀態碼仍要留著，不被蓋掉', async () => {
    const doFetch = responds(() => new Response('<html>Not Found</html>', { status: 404 }));

    await expect(askReading(KEY, TERM, doFetch)).rejects.toThrow('Gemini 回了 404：沒有附原因');
  });

  it('回應根本不是 JSON：讀不懂', async () => {
    const doFetch = responds(() => new Response('這不是 JSON', { status: 200 }));

    await expect(askReading(KEY, TERM, doFetch)).rejects.toThrow('讀不懂 Gemini 的回覆');
  });

  it('是 JSON 但挖不到那段文字：沒有回覆內容', async () => {
    const doFetch = responds(() => json({ candidates: [] }));

    await expect(askReading(KEY, TERM, doFetch)).rejects.toThrow('Gemini 沒有回覆內容');
  });

  it('挖到文字但它不是 JSON：模型答非所問', async () => {
    const doFetch = responds(() => reply('這個詞我不會'));

    await expect(askReading(KEY, TERM, doFetch)).rejects.toThrow('Gemini 回的不是 JSON');
  });
