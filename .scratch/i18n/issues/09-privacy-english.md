...
### 「沒有機器解法」那一段已經被票 11 推翻一半

上面 `## 兩份檔的同步問題沒有機器解法` 寫著「在兩份檔的開頭註解裡互相指名對方，是目前唯一的提醒手段」。
票 `11-privacy-sync-reminders.md` 裝了兩道 hook 之後那句不再成立：agent 改完檔會跳一句提醒，
commit 前 `commit-msg` 會擋。兩道都掃改動的內容找隱私訊號，不看檔名。

本文保留原樣不改，那是當時的決策紀錄。現況以 `../spec.md` 決定八為準。
標題那句「沒有機器解法」現在應該讀成「沒有**完整**的機器解法」——兩道 hook 都只答得出
「你可能該去看一眼」，答不出「這頁寫得對不對」。
