...
> **狀態已更新（2026-08-12）：規格已釘完，見 `spec.md` 與 `docs/adr/0013-interface-localization.md`，實作票在 `issues/01`–`10`。**
>
> **本檔以下是釘規格之前的交接內容**，保留作為背景與量測紀錄。有兩處已被規格推翻，讀的時候要注意：
>
> - **需求從四語改成三語，韓文砍掉了**（`spec.md` 決定十四）。理由是沒有人驗得了韓文，而翻譯品質沒有任何機器驗收條件擋得住。韓國市場沒有跟著砍，韓文裝置的介面落到英文
> - **「約 204 條介面字串」這個數字偏高**。它把 `gemini-reading.ts:67` 的 `INSTRUCTIONS`（送給 Gemini 的作業指示，約 11 條、3727 字元）算了進去，那是給模型讀的、絕對不能翻。實際約 193 條、3900 個中文字
>
> 四個難點的答案一律以 `spec.md` 為準。

**當時的狀態：需求已確定，規格未釘，一行 code 都還沒寫。**
