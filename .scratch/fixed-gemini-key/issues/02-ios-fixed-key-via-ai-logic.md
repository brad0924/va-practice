...
- ~~離線狀態下 iOS 新增卡片，讀音格留空、儲存不被擋。~~ **不必另外測，已由 2026-08-20 真機上那三次逾時涵蓋**——預填失敗的完整演出（格子留空、上方一行字、不跳框、不鎖按鈕、人工填完照樣存檔）當場就看到了，離線走的是同一段程式碼，只是換一句話。
  - 這條原本的措辭「儲存不被擋」與「讀音是必填格」自相矛盾：格子空著本來就存不下去。原意是 `ADR-0005` 的保底路徑——**預填失敗不要變成一道關卡**，人自己填完照樣存得下去。
...
**沒有的那一段**：這三段串起來時，手機畫面上真的一個字都沒有。要補的話是一輪 TestFlight——把餵給 SDK 的權杖改成上面那行亂寫的，裝上去試一次，改回來。

**實作者的判斷是不補**：剩下的風險是「App Check 壞掉時畫面出現一句不精準的話」，不是當機也不是掉資料，而且只在維護者自己弄壞設定時發生。**維護者尚未拍板**，要補的話是上面那一輪。
