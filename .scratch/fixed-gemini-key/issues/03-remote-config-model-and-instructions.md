...
### 2026-08-21 — 第三條也過了，票收掉

改名版的做法照走：兩個 key 改成別的名字、發布、刪 app 重裝、打詞條。**讀音預填照樣填得進來**，用的是程式碼裡那份預設模型。驗完名字已改回 `gemini_model` 與 `gemini_instructions` 並發布。

#### 驗收總表

| 驗收 | 結果 |
| --- | --- |
| `gemini_model` 改成不存在的模型名 → 讀音預填開始失敗 | 過（真機，08-20） |
| `gemini_instructions` 改一個字 → 送出去的提示詞真的變了 | 過（真機，08-20） |
| ~~飛航模式下全新安裝~~ → 拿掉那兩個 key，讀音預填仍用預設模型正常運作 | 過（真機，08-21，改名版） |
| 網頁版產物完全不受影響 | 過（`vite build` 產物裡 Remote Config 相關字串 0 個） |
| `npm test` 全綠 | 過（567 條） |

主控台狀態：兩個參數都是正式名字、正式值，已發布。

#### 留在紀錄裡的兩個缺口

**一、網路整個斷掉那條路沒有真機觀察。** 換了做法就換掉了這一段。它比驗過的那條更短——`refresh()` 丟出去就不管，失敗被 `.catch(() => {})` 吞掉，沒有任何一個 `await` 掛在它上面。靠讀程式碼，不靠真機。維護者知情選的，理由見上一則。

**二、票 05 還欠一句話。** Remote Config 抓設定前會先跟 Google 要一組 Firebase Installation ID，隱私權政策與 App Privacy 表都要交代。已在票 05 留下註記。
