...
- [-] ~~保險副本的救援行為驗過：清掉主儲存、重開，資料從副本回來~~ — **這一條作廢**。
      票 `07` 於 2026-08-27 拍板：保險副本在 React Native 版不留，因此沒有東西可驗。
      經過見下方 Comments。

## Comments

### 2026-08-27 — 驗收第 5 條作廢，票 `07` 拍板不留保險副本

保險副本在 React Native 版不做，正式定案。它防的是 iOS 清掉 WebView 那一層的網站資料，
這一版沒有 WebView，威脅不存在。因此驗收第 5 條沒有東西可驗，作廢而不是留著開。

那張票同時查到一件本票沒注意到的事：**MMKV 檔案損毀時的預設行為是整份丟掉**，
app 會開起來像剛裝好的。已改成 `recoveryStrategy: 'recover-on-error'`，
溯源寫在 `mobile/lib/storage-mmkv.ts` 的註解與 `ADR-0002` 的 2026-08-27 補充。
