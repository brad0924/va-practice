...
- [x] 鍵盤開著時「取消」按得到，不會白按一下
- [x] 詞條重複時擋得下來（`assertTermAvailable()`），訊息說得出撞到哪一張
- [x] `../hig-checklist.md` 補進表單與輸入相關條目，並逐項核過
- [x] **並排目測**：與 Apple 自家 app 擺一起，維護者判定不覺得舊
- [x] 日文九宮格組字到一半按 return 不會跳欄（見底下〈一條推論，寫下來讓真機去打〉）
...
### 2026-09-01 — 讀音預填在真機上通了

App Check 那條路整條走完：接線 OK、拿得到 App Attest 權杖、問得到模型、假名填進格子。
驗收第 3 條與「AI 讀音預填真的問得到」那一條**過了**。

三趟才通，病灶各不相同，記在這裡免得下一個人重走：

| 趟 | 症狀 | 病灶 |
| --- | --- | --- |
| 1 | pod 裝不起來 | Firebase 走 SPM 撞上靜態連結 |
| 2 | pod 裝不起來 | `GoogleUtilities` 那三個沒有 module map |
| 3 | 走成 DeviceCheck | factory 沒搶在 `FirebaseApp.configure()` 前面掛上 |

### 2026-09-01 — 接縫那兩顆鈕：上藍色、字級對齊網頁版

**票面原本寫「視覺上的符號維持小顆」，真機看過之後改掉了。** 維護者原話：
「不夠清楚」。灰色（`tertiaryLabel`）的 13 點符號在螢幕上看不出那裡可以按。

| 那一格 | 之前 | 現在 | 依據 |
| --- | --- | --- | --- |
| 顏色 | `tertiaryLabel`（灰） | `color.accent`（`systemBlue`） | 見下 |
| 字級 | `footnote`（13） | `headline`（17） | 網頁版 `.reading-seam` 是 `1.1rem`（17.6），差的 0.6 看不出來 |
| 按下去的圓角 | `TAP_SIZE / 2`（正圓） | 8 | 網頁版 `.reading-seam` 是 `0.5rem`，那邊是圓角方塊 |

**44 那一格一個點都沒動。** 撐大的只有看得到的符號；觸控區是 `B-01` 的下限，
與符號多大是兩件事。既有那條測試（「接縫的觸控目標是 44 點見方」）照樣綠。

#### 顏色為什麼不是網頁版那個 `#6ea8ff`

維護者拍板走 `systemBlue`，理由是這個 codebase 已經為「兩種藍」做過一次決定：
`./term.tsx` 的振假名用 `#6ea8ff`，而那段註解特別寫著它**刻意不用** `color.accent`
——「振假名按不下去，用同一個藍會把它講成一顆按鈕」。

接縫**是**按鈕，所以走的正是那個「按得下去」的藍。網頁版兩者同色，手機上刻意分開。
附帶好處是「提高對比」打開時它跟著調（`T-11`），寫死的色碼做不到。

> **這兩格不另外寫測試。** 它們取自 `./theme.ts` 的具名值，打錯字是型別錯誤，
> `tsc` 就擋下來了。卡片列表那六個桶色有測試守，是因為那邊是寫死的色碼——
> 沒有任何工具看得出「明天」拿到了「未來」那一格。

### 2026-09-01 — 十三條全過，收掉

真機把剩下四條驗完，全過，包含〈已知風險〉指名的那一條與並排目測。
狀態改 `done`。

> 上面〈驗收狀態〉那一段裡還留著幾個沒打勾的方框，**那是 2026-08-31 當下的快照，不改**
> ——與票 `15` 收掉時的做法一致。真正的驗收清單在檔案上方。

#### 那條「已知風險」的結局

票寫的是「44pt 的觸控方塊塞在字與字之間，會把詞條撐得很開⋯⋯撐開到不能看的話，
那時候再回頭談做法」。**沒有撐到不能看**，做法不必回頭談。

真機上唯一要改的是**看得清楚**：灰色的 13 點符號認不出那裡可以按，改成 `systemBlue`
加 17 點就解決了（見上一則）。那與尺寸無關——票原本連帶寫的「視覺上的符號維持小顆」
才是不成立的那一句。

#### 這張票留下兩樣要在別處收掉的東西

1. **`probeReading()` 與資料頁探針上那顆「試問一次讀音」。** 它是為了逼那三條靜默的路
   開口而加的，程式碼裡與這裡都寫著：資料頁那張票做好時一起消失。
2. **「壞了」與「它根本沒試」分不出來**這件事本身。`SilentError` 那條路（spec 決定十一）
   對使用者是對的，但這一輪為它燒掉兩趟真機——維護者自己也看不出差別。
   要不要留一個只有維護者看得到的出口，是 spec 層的決定，不歸這張票。

#### 下一頁

`../spec.md` 的頁面順序是**卡片列表 → 編輯 → 資料 → 統計**，前兩頁到此做完。
資料頁與統計頁還沒開票。
