...
- **編號** — 例如 `M-01`。後面的票要引用哪一條就寫編號。**編號只增不改**：作廢的條目留著、在條目欄標「（作廢）」，不要把號碼讓給新的一條。維護者決定不照某一條時，條目欄標「（刻意不照）」，條目本身照抄 Apple 的原意不改；「怎麼驗」那一欄寫本 app 改驗什麼、理由、出處票號。
- **條目** — 一句話，只講一件事。
...
| N-07 | tab 圖示用 SF Symbols，優先選填滿（filled）的版本（刻意不照） | Tab bars §Best practices | 看圖示是不是自訂圖檔。**本 app 改驗**：沒選中的 tab 是線框版、選中那格是填滿版，才算過。理由是與網頁版一致，維護者 2026-09-22 拍板（`ios-app/23`） |
