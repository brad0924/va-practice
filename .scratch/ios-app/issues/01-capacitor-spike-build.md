...
- [x] app 能安裝到實體 iPhone 並啟動，進入複習畫面
- [x] 能新增一張卡、複習它、關掉 app 再開，資料還在
- [x] 飛航模式下能啟動並完成一次複習
- [x] 既有測試全數通過（`npm test`），且一個測試檔都沒被修改
- [x] 網頁版 build 出來的行為與這張票之前一字不差
- [x] 上述八項災情逐項實測，結果寫進本檔 `## Comments`
...
2026-08-07 於實體 iPhone 上逐項實測完畢。`capacitor.config.ts` 維持 `ios.contentInset` 等所有預設值，沒有預先修補任何東西。

| # | 項目 | 要看什麼 | 結果 |
| --- | --- | --- | --- |
| 1 | 朗讀 | Web Speech 在 WKWebView 裡找不找得到日文語音？按鈕是否默默消失 | ⚠️ **與預期不同**，見下 |
| 2 | 雲端備份 | `fetch` 打 Firebase RTDB 是否成功？自訂 scheme 的來源有沒有被擋 | ✅ 通過。登入與推送皆無錯誤訊息，自訂 scheme 的來源沒有被擋 |
| 3 | 讀音預填 | `fetch` 打 Gemini API 是否成功（需自備金鑰） | ✅ 通過。填入金鑰後正確產出讀音 |
| 4 | 鍵盤 | 編輯畫面輸入時，鍵盤有沒有把版面頂壞、遮住必填格 | ✅ 通過，版面未被頂壞 |
| 5 | 安全區域 | 瀏海與底部橫條有沒有蓋住東西？既有的 `safe-area-bottom-gap` 在殼裡還成不成立 | ✅ 通過，既有處理在殼裡仍然成立 |
| 6 | 捲動 | 卡片列表、時間桶展開收合的手感有無明顯異常 | ✅ 通過，無明顯異常 |
| 7 | 離線 | 飛航模式下能否正常啟動與複習 | ✅ 通過 |
| 8 | 資料 | localStorage 讀寫是否正常，關掉 app 再開資料還在不在 | ✅ 通過。完全滑掉 app 再開，卡片與進度都在 |

**八項裡七項乾淨過關。WKWebView 沒有照出任何結構性問題**——這是這趟探路最重要的結論：Capacitor 包殼這條路是通的，`ADR-0002`（localStorage 為唯一真相來源）在殼裡完全成立，`fetch` 對 Firebase 與 Gemini 都沒有被自訂 scheme 的來源擋下。

#### 唯一的發現：朗讀（#1）——問題不是「有沒有」，是「好不好」

**實測結果與 spec 的預設相反。** spec 決定十五假設「WKWebView 裡日文語音的存在與否不可靠，按鈕會時有時無」。實際上：

- **朗讀按鈕正常顯示**，`hasJapaneseVoice()` 在 iOS WKWebView 裡回 `true`
- **聽得到日文**，念的也是振假名標注的讀音
- **但音質差**：接近機械音，重音（高低アクセント）聽起來不對

推測原因：iOS 的 Web Speech API 底層就是原生的 `AVSpeechSynthesizer`，但只拿得到 **compact（壓縮）品質**的語音，取用不到 enhanced／premium——後者需要使用者自行到系統設定下載。機械感是壓縮語音的典型特徵。

**這改變了票 07 的價值主張，但不改變它的做法**（該票第 15 行本來就預留了「不論結果如何，做法不變」）。差別在於：

- 決定十四（改用原生 `AVSpeechSynthesizer`）**依然成立，但理由從「為了有聲音」換成「為了聲音好聽」**——原生 API 可以明確指定 enhanced 品質
- 決定十五（讓按鈕永遠可用）**前提被推翻**：按鈕本來就永遠在，這個問題在 iOS 上不存在。改用原生後 `hasJapaneseVoice()` 恆為真仍是更強的保證，但那已不是這張票的動機
- **要誠實的一點**：重音問題未必修得好。日文的高低重音對合成器是難題，原生同樣是從假名推測，換過去不保證解決。票 07 的驗收不該把「重音正確」當成過關條件

已將以上寫進票 07 的 `## Comments` 與 `spec.md` 決定十五。
