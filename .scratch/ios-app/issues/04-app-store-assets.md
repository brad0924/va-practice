...
- [x] 隱私權政策頁面隨網頁版部署上線，有一個穩定可貼進 App Store Connect 的網址
      → **`https://brad0924.github.io/va-practice/privacy.html`**
      本分支併進 `main` 後由 `deploy.yml` 部署，push 後約 40 秒轉 200。
      線上那份已驗是修正後的版本（「密碼留在這台裝置上」「不經過任何第三方伺服器」
      「沒有提供刪除雲端那份備份的按鈕」三段都在，`va-practice:gemini` 鍵名確實不在）。
      `icon-1024.png` 同時一起上線（200）。
