...
- [x] iPhone 的「資料」畫面出現「語音診斷」按鈕，按下去印出四樣資訊
- [x] 網頁版看不到這顆按鈕，行為與本票之前一字不差
- [x] 朗讀改為跟著系統預設的日文語音走
- [x] 既有測試全數通過，且一個既有測試檔都沒被修改
- [x] **診斷結果寫進本票的 `## Comments`，並回填票 07 的驗收**
...
### 2026-08-11 — A／B 重比：兩邊聽起來不一樣了，鷹架已拆

**使用者回報：app 與網頁版的朗讀聽起來不一樣。** 這證實了「跟著系統選的那顆走」確實改變了實際念出來的語音——app 念的是 O-ren，網頁版念的是 Kyoko，與診斷印出來的完全對得上。

至此本票的規則改動有了執行期的證據，決定一留下。
...
| 檔案 | 證據 |
| --- | --- |
| `src/ui/data-view.ts` | `git diff 4245f7b -- src/ui/data-view.ts` 為空（逐字相同） |
| `src/lib/speech-native.ts` | `git diff 4245f7b -- src/lib/speech-native.ts` 為空（逐字相同） |
| `ios/App/App/SpeechPlugin.swift` | `describeVoices` 與 `describe` 兩支移除，`pluginMethods` 回到只剩 `speak`；留下的是選語音的新規則 |

網頁版 bundle 從 57.35 kB 回到 **56.90 kB**，與加鷹架之前一致。全部測試 391 過，typecheck 乾淨，兩個 build 與 `cap sync ios` 都過。

#### 這次學到的：猜測要當成待驗事項，不是結論

本票的「現況」段落押了兩個猜測，**兩個都錯**——O-ren 拿得到，系統偏好也讀得到。它們沒有造成傷害，唯一的理由是當初把它們寫成「探針要回答的問題」，而不是寫成前提去改程式。

同一個模式已經連續三次奏效（票 13 的保險副本、票 12 的分享單、本票）：**在看不到除錯器的環境裡，一顆用完就拆的按鈕，成本是一趟 CI，換到的是事實。**
