...
- [x] iPhone 上按「匯出備份」跳出 iOS 分享單
- [x] 存到「檔案」app 後，檔名是 `jlpt-cards-YYYY-MM-DD.json`
- [x] 該檔案在網頁版能成功「匯入備份」，卡片與進度完整
- [x] 網頁版匯出的檔案在 iOS 上也能成功匯入（雙向互通）
- [x] 在分享單上按取消，不跳任何錯誤、畫面照常
- [x] iPhone 上按「匯入備份」跳得出檔案選擇器（見「待確認」；壞掉的話回頭修訂本票）
- [x] 網頁版的匯出行為與本票之前一字不差
...
## 2026-08-10 — 真機驗證通過，九條驗收全數成立

六條需要真機的驗收在 iPhone 上一次過關，沒有回頭修任何東西。

這一下同時證實了三件寫程式時只能靠推理的事：

1. **`download` 屬性確實是原因**——換掉它之後同一顆按鈕就有反應了。票裡把三個嫌疑（不認 `download`、連結沒掛進 DOM、`revokeObjectURL` 收太早）併成一個修法、不去分辨，事後看是對的：真兇就是第一個。
2. **`isCancelled()` 認得出真正的取消**——滑掉分享單沒有冒出紅字。這是整條路上唯一靠字串比對撐住的地方，在此之前沒有任何真機證據。
3. **雙向互通成立**——兩邊產出的備份檔可以互相匯入，`exportBackup()` 那一串原封不動地過了分享單這一趟，沒有被編碼或換行搞壞。

### 「待確認」那一條的答案：匯入沒壞

**檔案選擇器正常跳出，匯入成功。** `<input type="file">` 加 `click()` 在 WKWebView 裡是好的——只有 `<a download>` 不被支援。因此本票範圍不必擴大，也不需要回頭修訂。

順帶記下這條分界，日後在 WebView 裡碰到類似問題時值得先想起來：**檔案「進來」的那條路 iOS 是通的，「出去」的那條才需要原生手段。**

### 維護者待辦（已完成）

1. ~~手動觸發 `Build iOS and upload to TestFlight`~~
2. ~~真機上驗六條驗收，其中「按取消不跳錯誤」務必真的按一次取消~~
3. ~~驗掉「待確認」：按「匯入備份」看檔案選擇器跳不跳得出來~~
